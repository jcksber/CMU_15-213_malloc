...
/*  NOTE!
 * Header:    [  MSB(1) |    LSB's(3)   ]
 * Footer:    [  MSB(1) |    LSB's(3)   ] (identical)
...
void *mmt_malloc(size_t size) 
{
  ...
  if (size <= DSIZE)
    asize = 2*DSIZE; // 8 bytes: hdr & ftr, <= 8 bytes: content
  else
 ...
 */
void mmt_checkheap(int verbose)  
{ 
}
