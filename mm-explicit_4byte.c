 ...
 *         A [ HDR |    PAYLOADS   | FTR ]
 * Bytes:      4      4        4      4   ==> MIN_BLK_SIZE = 16 bytes
 */
...
#define MIN_BLK_SIZE 16       /* next & prev ptr's 8 bytes for 64-bit */
...
/* Convert 4 byte next & prev values into their 8 byte pointers */
#define GETP(p)            ( (((u_llong)0x1) << 35)+((u_llong)(*(u_int *)(p))) )
#define GET_NEXT_FREEP(bp) ( GETP(NEXT_FREEP(bp)) )
#define GET_PREV_FREEP(bp) ( GETP(PREV_FREEP(bp)) )

/* Put 4 byte value of the next & prev pointers at bp */
#define PUTP(p, bp)        ( *(u_int *)(p) = (u_int)((u_llong)(bp)) )

/* ## SEGREGATED LIST MACROS ## */
#define SEGx                 /* use a segregated list? */
...
static void seg_insertb(void *fp, void *bucket);
...
static void *coalesce(void *bp) 
{
  ...
  #ifdef SEG
    bucket = seg_search(size);
    seg_insertb(bp, bucketh);
  #else
    insertb(bp);
  #endif
...
static void seg_removeb(void *fp)
{
  ...
  //   //PUTP(GETP(PREV_BLKP(bp))+4, GETP(bp+4));
...
static void insertb(void *fp)
{
  if (free_listp) {
    /* previous free list start now needs prev ptr */
    PUTP( PREV_FREEP(free_listp), fp ); 
    /* set next free ptr to old start of list */
    PUTP( NEXT_FREEP(fp), free_listp );
  }
  else 
    PUTP( NEXT_FREEP(fp), NULL ); 

  /* new first free block in list */
  PUTP( PREV_FREEP(fp), NULL ); 
  /* reset start of free list */
  free_listp = (char *)fp;       
}
...
static void removeb(void *fp)
{
  char *next = (char *)GET_NEXT_FREEP(fp);
  char *prev = (char *)GET_PREV_FREEP(fp);

  /* Case: previous block exists */
  if (*prev) {     /* adjust previous free block's next pointer */
    if (*next)
      PUTP( NEXT_FREEP(GET_PREV_FREEP(fp)), GET_NEXT_FREEP(fp) ); 
  }
  /* Case: .. doesn't exist */
  else 
    free_listp = (char *)GET_NEXT_FREEP(fp); /* adjust start of free list */
  if (*next)      /* adjust next free block's previous pointer */
    PUTP( PREV_FREEP(GET_NEXT_FREEP(fp)), GET_PREV_FREEP(fp) );
}
